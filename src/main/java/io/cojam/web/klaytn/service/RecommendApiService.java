...
public class RecommendApiService {
    @Value("${app.sdk-enclave.recommendPassphrase}")
    private String recommendPassphrase;
    ...
    public TransactionReceipt sendMasterWalletTokenTransferTransaction(String to, BigInteger amount, String ticker) {
        return serviceTemplate(() -> {
            SendTokenTransferRequest sendTokenTransferRequest = SendTokenTransferRequest.builder()
                    .passphrase(recommendPassphrase)
                    .to(to)
                    .amount(amount)
                    .ticker(ticker)
                    .build();
    ...
    private <T> T serviceTemplate(RecommendApiService.ServiceCallback<T> callback) {
        try {
